...
export async function middleware(request: NextRequest) {
  ...
  if (!userId && path !== "/" && path !== "/login" && path !== "/latest_script.sh" && path !== "/install.sh" && path !== "/script.sh" && path !== "/pid.py" && path !== "/auth-log.py"  && path !== "/scan.py" && path !== "/sensors.py"  && path !== "/scripts/GG.ps1") {
