...
import { assignUsersToEmailTemplate, createEmailTemplate, removeUsersFromEmailTemplate, updateEmailTemplate } from "@/app/actions/email-template-actions"
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                The subject line of the email. You can use placeholders like user, rule, etc.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea 
                  placeholder="Dear {{user}}, We detected high CPU usage on {{device}} at {{time}}..." 
                  className="min-h-[200px]"
                  {...field} 
                />
              </FormControl>
              <FormDescription>
                The body of the email. You can use placeholders like user, rule, device, time, etc.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
          <FormField control={form.control} name="assignedUsers" render={({ field }) => (
          <FormItem>
            <FormLabel>Assign Users</FormLabel>
            <FormControl>
              <MultiSelect options={users.map(user => ({ label: user.username, value: String(user.id) }))} value={field.value || []} onChange={field.onChange} placeholder="Select users..." />
            </FormControl>
            <FormMessage />
          </FormItem>
        )}/>
